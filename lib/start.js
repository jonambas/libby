import webpack from 'webpack';
import path from 'path';
import HtmlWebpackPlugin from 'html-webpack-plugin';
import WebpackDevServer from 'webpack-dev-server';
import open from 'open';
import FriendlyErrorsWebpackPlugin from 'friendly-errors-webpack-plugin';

function makeWebpackConfig(config, options) {
  const libraPath = path.resolve('..');
  const includePaths = [path.resolve(libraPath, 'src'), path.resolve('./src')];

  const devServerEntries = options.production
    ? []
    : [`webpack-dev-server/client?http://localhost:${config.port}`, 'webpack/hot/dev-server'];

  const generatedConfig = {
    mode: options.production ? 'production' : 'development',
    entry: {
      main: [...devServerEntries, path.resolve(libraPath, 'src/ui/index.js')],
      preview: [path.resolve(libraPath, 'src/preview/index.js')]
    },
    output: {
      filename: '[name].[hash].js',
      path: path.resolve(config.cwd, config.outputPath)
    },
    resolve: {
      extensions: ['.tsx', '.ts', '.jsx', '.js', '.json'],
      alias: {
        __LIBRA_CONFIG__: path.resolve(config.cwd, 'libra.config.js'),
        react: path.resolve(config.cwd, 'node_modules/react')
      }
    },
    module: {
      rules: [
        {
          test: /\.js?$/,
          include: includePaths,
          use: {
            loader: 'babel-loader',
            options: {
              presets: ['@babel/preset-env', '@babel/preset-react'],
              plugins: ['@babel/plugin-proposal-class-properties']
            }
          }
        }
      ]
    },
    plugins: [
      new HtmlWebpackPlugin({
        title: config.title ? `Libra | ${config.title}` : 'Libra',
        chunksSortMode: 'none',
        chunks: ['main'],
        filename: 'index.html'
      }),
      new HtmlWebpackPlugin({
        title: 'iframe',
        chunksSortMode: 'none',
        chunks: ['preview'],
        filename: 'iframe.html'
      }),
      ...(options.production
        ? []
        : [new webpack.HotModuleReplacementPlugin(), new FriendlyErrorsWebpackPlugin()])
    ]
  };

  return generatedConfig;
}

const start = async (config, callback) => {
  // Make dev webpack config
  const webpackConfig = await makeWebpackConfig(config, {
    production: false
  });

  const webpackDevServerConfig = {
    hot: true,
    stats: {},
    noInfo: true,
    quiet: true,
    clientLogLevel: 'none',
    compress: true,
    inline: true,
    watchOptions: { ignored: /node_modules/ }
  };

  const compiler = webpack(webpackConfig);
  const devServer = new WebpackDevServer(compiler, webpackDevServerConfig);

  const { port, openBrowser } = config;

  devServer.listen(port, '0.0.0.0', (...args) => {
    const [err] = args;

    if (!err && openBrowser) {
      open(`http://localhost:${port}`);
    }

    if (typeof callback === 'function') {
      callback(...args);
    }
  });
};

export { start };
